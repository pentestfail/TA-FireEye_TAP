...
def collect_events(helper, ew):
    ...
    try:
        ...
        helper.log_debug("[" + opt_instanceid + "] TAP Alerts - Last successful checkpoint time: " + str(last_status))
    except Exception as e:
        helper.log_error("[" + opt_instanceid + "] TAP Alerts - Unable to retrieve last execution checkpoint!")
        raise e
    ...
    url = "https://" + opt_environment + ".fireeye.com/tap/id/" + opt_instanceid + "/api/v1/alerts"
    method = 'GET'
    ...
    try:
        ...
        if obj is None:
            helper.log_info("[" + opt_instanceid + "] No new alerts retrieved from TAP.")

        # Iterate over alerts in array & index
        i=0
        for alert in obj.get("alerts"):
            singleAlert = (obj.get("alerts")[i])
            singleAlert['tap_instance'] = opt_instanceid
            singleAlert['tap_environment'] = opt_environment
        ...
        helper.log_debug("[" + opt_instanceid + "] TAP Alerts - Storing checkpoint time: " + current_status)

    except Exception as error:
        helper.log_error("[" + opt_instanceid + "] TAP Alerts - An unknown error occurred!")
        raise error